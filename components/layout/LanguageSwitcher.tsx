...
export function LanguageSwitcher({ variant = "light", onSwitched }: Props) {
  ...
  const triggerClass = isDark
    ? `inline-flex h-9 min-w-[3rem] items-center justify-center gap-1.5 rounded-full border px-3 font-mono text-[11px] uppercase tracking-[0.2em] transition-all duration-200 ${
        open
          ? "border-[#e0af6e] bg-[#e0af6e]/10 text-[#e0af6e]"
          : "border-[#e0af6e] text-[#e0af6e] hover:bg-[#e0af6e]/10"
      }`
  ...
  const itemClass = isDark
    ? "flex w-full items-center justify-center px-3 py-2 font-mono text-[11px] uppercase tracking-[0.2em] text-white/80 transition-colors hover:bg-[#e0af6e]/10 hover:text-[#e0af6e]"
