...
var SEO_V4={
  ...
  '/estimation':{
    title:'Estimation immobilière Luxembourg — MAPA Property',
    desc:'Estimation indicative gratuite en ligne ou avis de valeur vénale professionnel sur dossier. Données STATEC et Observatoire de l\'Habitat. Estimation EVS.',
    h1:'Estimation de votre bien au Luxembourg'
  },
  '/mandat-recherche':{
...
function renderBiensInto(gridId,emptyId,scope){
  ...
  if(scope==='location'){
    ...
    /* FIX36 : prioriser properties_offmarket reels charges via offmarket-public.js */
    if(typeof window.OFFMARKET_PROPS!=='undefined'&&window.OFFMARKET_PROPS&&window.OFFMARKET_PROPS.length){
      list=window.OFFMARKET_PROPS;
    }else{
    ...
    var omListA=(typeof window.OFFMARKET_PROPS!=='undefined'&&window.OFFMARKET_PROPS&&window.OFFMARKET_PROPS.length)?window.OFFMARKET_PROPS:DEMO_OFFMARKET;
    var base=(PROPS&&PROPS.length)?PROPS.concat(omListA):[DEMO_BIEN,DEMO_LOCATION].concat(omListA);
...
function renderFeaturedBiens(){
  ...
  if(carousel){
    ...
    function initFavSwiper(){
      ...
      var _favCount = feats.length;
      var _favPerView = _favCount >= 6 ? 3 : (_favCount >= 4 ? 2.2 : 1.5);
      window._favSwiper = new Swiper('.fav-swiper', {
        slidesPerView: 1.15,
        spaceBetween: 18,
        loop: _favCount >= 3,
        loopedSlides: Math.max(_favCount * 3, 12),
        autoplay: { delay: 3500, disableOnInteraction: false, pauseOnMouseEnter: true, reverseDirection: false, stopOnLastSlide: false, waitForTransition: true },
        speed: 800,
        grabCursor: true,
        watchOverflow: true,
        speed: 800,
        ...
        breakpoints: {
          768: { slidesPerView: Math.min(2.2, _favCount), spaceBetween: 22 },
          1024: { slidesPerView: Math.min(3, _favCount), spaceBetween: 28 },
          1400: { slidesPerView: Math.min(4, _favCount), spaceBetween: 28 }
        }
      });
...
window.openBien=function(id){
  ...
  var omList=(typeof window.OFFMARKET_PROPS!=='undefined'&&window.OFFMARKET_PROPS&&window.OFFMARKET_PROPS.length)?window.OFFMARKET_PROPS:DEMO_OFFMARKET;
  var list=(PROPS&&PROPS.length)?PROPS.concat(omList):[DEMO_BIEN,DEMO_LOCATION].concat(omList);
  var b=null;
...
function renderBienDetail(b){
  ...
  '<div class="bd-body">'+
    ...
    '<aside class="bd-side">'+
      ...
      '<div class="bd-agent">'+
        '<div class="bd-ag-av"><img src="https://dutfkblygfvhhwpzxmfz.supabase.co/storage/v1/object/public/photos/IMG_2461.jpg" alt="Julien Brebion" loading="eager" onerror="this.style.display=\'none\';this.parentNode.innerHTML=\'<div style=&quot;width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#b89448,#a07040);color:#fff;font-family:\'Cormorant Garamond\',serif;font-size:22px;font-weight:700&quot;>JB</div>\'"></div>'+
  ...
  if(isMaison){
    ...
    r.innerHTML=
      ...
      '<div style="margin-top:14px;text-align:center">'+
        '<button class="btn btn-navy" onclick="closeM(\'m-estimation\');openSvc(\'m-contact\')" style="width:100%">'+
          (T['mod.est.cta.pro']||'✦ Demander une estimation professionnelle →')+
        '</button>'+
      '</div>';
    return;
  }
...
function filterSimilarStrict(b){
  if(!b)return [];
  var targetType=b.property_type||b.category||b.type||'';
  var targetCountry=b.country||'';
  var targetPrice=parseFloat(b.price_value||b.price||b.price_indicative||0);
  /* FIX36 : range strict +/- 25% sur prix reel */
  var minPrice=targetPrice>0?targetPrice*0.75:0;
  var maxPrice=targetPrice>0?targetPrice*1.25:Infinity;

  /* FIX36 : sources unifiees - properties + properties_offmarket publies */
  var sources=[];
  if(typeof PROPS!=='undefined'&&PROPS&&PROPS.length)sources=sources.concat(PROPS);
  if(typeof window.OFFMARKET_PROPS!=='undefined'&&window.OFFMARKET_PROPS&&window.OFFMARKET_PROPS.length){
    /* Off-market : seulement les is_published=true (deja filtre cote serveur normalement) */
    var omPub=window.OFFMARKET_PROPS.filter(function(o){return o&&o.is_published!==false;});
    sources=sources.concat(omPub);
  }
  if(!sources.length)return [];

  return sources.filter(function(p){
    if(!p||String(p.id)===String(b.id))return false;
    var pType=p.property_type||p.category||p.type||'';
    var sameType=(pType&&targetType&&pType===targetType);
    if(!sameType)return false;
    if(targetCountry&&p.country&&p.country!==targetCountry)return false;
    if(targetPrice>0){
      var pv=parseFloat(p.price_value||p.price||p.price_indicative||0);
...
function filterSimilarLarge(b){
  if(!b)return [];
  var targetType=b.property_type||b.category||b.type||'';
  /* FIX36 : meme unification que strict */
  var sources=[];
  if(typeof PROPS!=='undefined'&&PROPS&&PROPS.length)sources=sources.concat(PROPS);
  if(typeof window.OFFMARKET_PROPS!=='undefined'&&window.OFFMARKET_PROPS&&window.OFFMARKET_PROPS.length){
    var omPub=window.OFFMARKET_PROPS.filter(function(o){return o&&o.is_published!==false;});
  ...
  return sources.filter(function(p){
    if(!p||String(p.id)===String(b.id))return false;
    var pType=p.property_type||p.category||p.type||'';
    return pType&&targetType&&pType===targetType;
  });
}
...
window.openSimilarModal=function(){
  ...
  /* FIX36 : seuil 3 - message + CTA si peu de resultats */
  var SIMILAR_MIN_THRESHOLD=3;
  var T=(I18N&&I18N[CURLANG])||{};
